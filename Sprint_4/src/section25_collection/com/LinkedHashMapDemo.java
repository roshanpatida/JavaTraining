...
//               1 System.out.println(tm.compute(Integer.SIZE, remappingFunction)); // its not working
             tm.putIfAbsent(4, "hello");// if any value is not present so 
                       // if key and value is not present so we can import on that perticular place
                       // but its present so it will not update and print old value
