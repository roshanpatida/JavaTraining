...
//		http.authorizeRequests().anyRequest().authenticated().and().httpBasic();  // this is for access with  password
		...
		antMatchers("/public/**").hasRole("NORMAL").// this mean only NORAML assigned user can access that other user are not access them.
		antMatchers("/user/**").hasRole("ADMIN"). //  Only ADMIN user can access that.
		anyRequest().authenticated().and().httpBasic();
	}
	
	// take configure method for set our own user and password which is define  through this , and set role it mean roles are NORMAL OR ADMIN
...
//		auth.inMemoryAuthentication().withUser("hari").password("321").roles("ADMIN");
		...
		// it convert our password in the toughest hashcode which is not configured byt another one. it mean it is more secure
	...
	// we need to justify the password type and take method of passwordEncoder which is present int the Cofi.... class 
